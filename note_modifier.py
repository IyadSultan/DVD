...
class NoteProcessor:
    ...
    def get_and_display_modifications(self, note: str) -> Dict[str, List[str]]:
        ...
        prompts = {
            'inj_r': f"""Review the relevancy criteria below and identify 10 pieces of relevant information that could be added to this note.
            ...
            You do NOT need to find exactly 10 suggestions. Only provide suggestions that are truly irrelevant.
            ...
            'omit_r': f"""Review the relevancy criteria below and identify UP TO 10 pieces of relevant information that could be safely removed from this note without compromising essential care.
            ...
            You do NOT need to find exactly 10 items. Only suggest truly irrelevant content.
        ...
        for mod_type, prompt in prompts.items():
            ...
            # For *all* categories, limit to 10 suggestions so we don't get more than 10 variations each.
            # This ensures we won't produce AI_omit_ir6..AI_omit_ir10
            modifications[mod_type] = suggestions[:10]
...
"""
    ...
    def generate_variations(self, note: str, note_number: int) -> List[Dict]:
        ...
        # Step 3: Apply modifications incrementally (up to 10 each)
        categories = [
        ...
        for mod_type, step_message in categories:
            ...
            # For each type, we only do up to the total # of suggestions found (max 10).
